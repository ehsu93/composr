...
public class FrequencyRecorder {
    ...
    public FrequencyArray fArray = new FrequencyArray(50);
    private Context ctx;
    ...
    public FrequencyRecorder (Context ctx) {
        this.ctx = ctx;
    }
    ...
    public float getMedian() {
        ...
        int lo = 0;
        int hi = this.fArray.frequencies.length;
        int mid = lo + (hi - lo)/2;
...
//        if(fArray.addAndGetFreq(freq) != -1f) return;
    ...
    public class FrequencyArray{
        ...
        public String toString(){
            String ret = "\n\t";
            for (int i = 0; i < this.frequencies.length-1; i++) {
                ret += String.valueOf(this.frequencies[i]) + "\n\t";
            }
            return ret;
        }
        ...
        public float[] cleanFreqs(){
            ...
            for( int i=0;  i<this.frequencies.length;  i++ ) {
                if (this.frequencies[i] != 0 && this.frequencies[i] != -1.0)
                    this.frequencies[j++] = this.frequencies[i];
            }
