...
public class FrequencyRecorder {
    ...
    private FrequencyArray fArray = new FrequencyArray(10000000);
    private Context ctx;
    ...
    /* accidentally did the same thing at the same time as Eric
    public void writeFrequencies(){
        String filename = "frequencies.txt";
        File file = this.ctx.getDir(filename, 0);
        try {
            BufferedWriter output = new BufferedWriter(new FileWriter(file));
            output.write(fArray.toString());
            output.close();
